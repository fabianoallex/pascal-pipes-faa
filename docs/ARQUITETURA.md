...
Motivação: um app que loga conexões (auditoria, diagnóstico) só tinha `AConnId` para
identificar quem conectou — um número opaco, sem significado fora do processo. Em
`ptTcp`/`ptTls`, mesmo sem mTLS, o IP do par já está disponível no socket; só faltava expor.

### 19.1 `TryPeerAddress` no contrato abstrato, não um hack só do servidor

Segue o MESMO padrão de `TryPeerIdentity` (§ver `TPipeEndpoint` em `Pipes.Transport.pas`):
método virtual na classe abstrata, `Result := False` por padrão, e cada backend sobrescreve
onde faz sentido. `ptLocal` (Windows `TPipeWinEndpoint`, POSIX `TPipePosixEndpoint` sobre um
fd `AF_UNIX`) não sobrescreve nada — herda o `False` — porque Named Pipe/UDS não tem
conceito de endereço de rede, só um handle/fd local. `TPipeTlsEndpoint.TryPeerAddress`
sempre delega ao endpoint TCP de baixo (`FInner`), nos dois backends (Schannel/OpenSSL) e
mesmo ANTES do `Handshake` — ao contrário de `TryPeerIdentity`, endereço não é conteúdo de
certificado, é propriedade do socket.

### 19.2 `TPipeRawSockAddrIn`/`TPipeRawSockAddrIn6`: struct declarada à mão, e por quê

`getpeername` devolve um `sockaddr` cujo layout real depende da família (`AF_INET` vs.
`AF_INET6`), e a unit de socket de cada compilador tipa isso de formas incompatíveis entre
si — o mesmo problema que já levou `Pipes.Transport.Tcp.pas` a declarar `TPipeAddrInfo` à
mão para `getaddrinfo` em vez de confiar na `WinSock2`/`Sockets` (ver o cabeçalho daquela
unit). A solução aqui é a mesma: `TPipeRawSockAddrIn`/`TPipeRawSockAddrIn6`/
`TPipeRawSockAddrStorage` (`Pipes.Transport.pas`) são o layout de bytes cru — família em
`Word`, porta em network byte order, endereço em array de bytes — que é **idêntico** entre
Windows e POSIX (só o VALOR da constante de família do IPv6 diverge: 23 no Windows, 10 no
Linux, cada backend com a sua). `pipe_getpeername` é declarado localmente em cada backend
(`Pipes.Transport.Tcp.pas` para Windows, `Pipes.Transport.Posix.pas` para POSIX), mesmo
idioma de `pipe_bind`/`pipe_connect`: o endereço trafega como ponteiro opaco + tamanho, para
o buffer caber qualquer família que o kernel devolva.

As structs e `PipeFormatIPv6` (formata sem a compressão `::` do formato canônico — mais
verboso, mas ainda um IPv6 válido e reconectável, suficiente para log) ficam em
`Pipes.Transport.pas` — não em `Pipes.Transport.Tcp.pas`, mesmo essa sendo a unit que
implementa o backend TCP do Windows — porque `Pipes.Transport.Posix.pas` (o backend POSIX)
também precisa delas para o PRÓPRIO `TryPeerAddress`, e as duas units já dependem de
`Pipes.Transport`.

### 19.3 A mesma classe serve UDS e TCP no POSIX: a família é o discriminador

`TPipePosixEndpoint` (`Pipes.Transport.Posix.pas`) embrulha tanto um socket `AF_UNIX`
(`ptLocal`) quanto um `AF_INET`/`AF_INET6` (`ptTcp`, montado em `Pipes.Transport.Tcp.pas` e
com a posse passada para esta classe — ver o cabecalho daquela unit) — é a MESMA classe,
sem um flag ou subclasse para distinguir a origem do fd. `TryPeerAddress` não precisa desse
flag: chama `getpeername` incondicionalmente e só reconhece `PIPE_AF_INET`/`PIPE_AF_INET6`
no `case`; um socket `AF_UNIX` devolve uma família diferente dessas duas e cai no `else`
(`Result := False`) pelo mesmo mecanismo, sem código extra. A checagem de família FAZ o
papel do discriminador que uma segunda classe faria, com uma linha a menos.

### 19.4 `FAddresses`/`FAddressOrder`: mesmo ciclo de vida de `FIdentities`, propositalmente duplicado

`TPipeServer.PublishEstablished` (chamada pela reader thread após o `Handshake`, antes de
`OnClientConnected`) agora também chama `AConn.FEndpoint.TryPeerAddress` e guarda o
resultado num dicionário PRÓPRIO (`FAddresses`/`FAddressOrder`), com o MESMO despejo pelo
mais antigo e o MESMO teto `PIPES_RECENT_IDENTITIES` de `FIdentities`/`FIdentityOrder` — a
razão de existir é idêntica: o endereço precisa sobreviver a `OnClientDisconnected` para
responder "de onde veio quem saiu?", e a limpeza da conexão não tem ordem garantida em
relação a esse evento (mesmo racional de `FIdentities`, ver o cabeçalho de
`Pipes.Server.pas`). A decisão foi duplicar o dicionário/lista/despejo em vez de generalizar
os dois num único cache de metadados: é mais código no total, mas cada mecanismo continua
aditivo e isolado — o padrão que `TPipeKeyedDispatcher` (§15.2) e outras extensões desta lib
já seguem, em vez de reabrir uma estrutura que já funciona para acomodar um segundo dado.

### 19.5 Cobertura de plataforma: Windows e POSIX/Linux implementados, Android fora desta rodada

Windows e POSIX (Linux) têm `TryPeerAddress` implementado e verificado — FPC/Win64
(`PipesIntegrationTestsFpc.exe`, suíte `Pipes.PeerAddressTests`) e Delphi/DUnitX. O ramo
POSIX **compila** (mesma disciplina de sempre: os símbolos são declarados à mão, sem
depender de nenhuma particularidade de versão da unit `Sockets`), mas **não foi executado**
nesta rodada — esta máquina de desenvolvimento não tem toolchain Linux, mesma limitação já
registrada para outras partes do POSIX (ver §16, descoberta LAN). `Pipes.Transport.Android`
não ganhou `TryPeerAddress` nesta rodada: é um eixo de plataforma à parte (§13), e o pedido
que motivou esta feature era de log num servidor desktop.

### 19.6 Testes

Integração (`Pipes.PeerAddressTests`, dual): cliente `ptTcp` conectado devolve
`'127.0.0.1:<porta efêmera>'` (só confere que a porta é um número > 0, nunca um valor fixo —
quem escolhe é o SO); `ptLocal` devolve `False` com string vazia; `AConnId` inexistente
devolve `False` (mesmo caso de `ConnectionStats`); e o endereço continua consultável DEPOIS
de `OnClientDisconnected` disparar, provando a sobrevivência do §19.4. As três primeiras
esperam `ServerStats.ClientCount` alcançar 1 antes de ler `ClientIds` — `Connect` do lado
cliente devolve antes de `PublishEstablished` rodar no servidor, uma corrida real que
`Pipes.StatsTests` já evita do mesmo jeito (`WaitCount` antes de qualquer leitura de
`ClientIds`/`Stats`).

## 20. Confirmação de entrega por assinante (`OnDelivered`/`OnDeliveryFailed`)

Motivação: um app de produção com muitos assinantes (dashboard/PDV) precisava logar, por
destinatário, se uma publicação do PRÓPRIO servidor (`Publish`/`PublishBatch`, ao vivo ou
replay de retido) chegou ao SO do lado daquela conexão — o log de `Publish` (uma chamada, do
lado de quem publicou) não distingue "ninguém assinava" de "assinava e o Write falhou", e
essa distinção é exatamente o que se precisa numa fase de implantação para responder a uma
inconsistência relatada em campo. `OnPublish` não serve: existe para o CLIENTE publicando,
não para o servidor sendo o publicador.

### 20.1 Dois eventos, não um com flag de sucesso

Decisão do usuário, com trade-off explícito: um único `OnDelivered(..., ASuccess: Boolean)`
seria menos código, mas obriga todo handler a filtrar o caso que não interessa a cada log
(sucesso vai para métrica, falha vai para alerta — raramente o mesmo handler quer as duas).
Dois eventos com assinaturas distintas (`TPipeTopicEvent` para sucesso — reaproveitando o
MESMO tipo de `OnPublish`/`OnTopicMessage`, já que os campos são idênticos; um
`TPipeDeliveryFailedEvent` novo só para a falha, com `AError` extra) deixam quem só quer
alertar sobre falhas assinar UM evento, sem filtrar nada.

### 20.2 "Entregue" é até o SO, nunca ACK de aplicação

`ARetained` aqui é o MESMO sentido do lado servidor de `TPipeTopicEvent` usado por
`OnTopicMessage` do cliente: `False` = entrega ao vivo, `True` = replay de valor retido na
assinatura. O nome do evento é deliberadamente `OnDelivered`, não `OnAcked` ou
`OnConfirmed`: dispara quando `SendFrame`/`SendFrames` retorna sem exceção, isto é, quando o
payload passou para o buffer do socket/pipe do SO — o mesmo corte que `Stats`/
`BytesSentWire` já fazem (§11/§17.6). O protocolo NUNCA teve ACK de aplicação (nenhum
`pfkAck`, nenhuma correlação de volta do cliente); prometer confirmação de processamento
seria uma mentira de API. Um cliente que trava ou descarta a mensagem depois de recebida no
socket continua invisível para este evento — isso é o limite consciente do recorte.

### 20.3 Onde dispara: FanOut, SendRetained e PublishBatch — sempre no laço de envio, fora do lock

Os três pontos de fan-out já existentes ganharam o disparo, sem mudar a forma como decidem
QUEM recebe (isso continua §9.2: casamento de tópico é código puro sob `FConnLock`, sem IO
nem alocação):

- `FanOut` (publicação ao vivo, uma única `TPipeFrame` reaproveitada entre conexões): cada
  `LConn.SendFrame` já estava num `try/except` individual por conexão (para uma conexão
  caindo não interromper o fan-out das demais); o disparo entra nesse mesmo `try/except`,
  sucesso chama `DispatchTopicEvent(FOnDelivered, ...)`, falha chama
  `DispatchDeliveryFailedEvent(FOnDeliveryFailed, ..., E.Message)`. Sempre `ARetained = False`
  — é o MESMO frame que sai com `PIPE_FLAG_RETAIN` desligado no fio (§9, "Sai SEMPRE com
  PIPE_FLAG_RETAIN desligado").
- `SendRetained` (replay na assinatura, `Length(LTopics)` valores num único `SendFrames`
  atômico — ver §9.6, "ou chega o replay inteiro ou não chega nenhum"): a atomicidade do
  WRITE não muda; o que muda é que, depois do `SendFrames` retornar (sucesso OU exceção), um
  laço dispara um evento POR TÓPICO retido — cada valor guardado continua sendo uma entrega
  logicamente distinta para quem loga observabilidade, mesmo tendo saído num Write só.
  Sempre `ARetained = True`.
- `PublishBatch`: o subconjunto de frames por conexão (`TPipeConnFrameBatch`) ganhou dois
  arrays paralelos a `Frames` — `Topics`/`Payloads` — só para essa correlação; depois do
  `SendFrames` da conexão, um laço dispara um evento por ITEM do lote que casou. Sempre
  `ARetained = False`, mesmo quando o item pediu `Retain = True`: isso é sobre o VALOR
  guardado (o que `StoreRetained` já resolveu antes do fan-out), não sobre esta entrega ser
  catch-up — mesmo raciocínio de `FanOut`.

Em nenhum dos três a conexão é derrubada por causa da falha: o reader dela já vai notificar
a queda via `OnClientDisconnected`, sem ligação com qual publicação específica falhou — é
exatamente essa correlação ausente que `OnDeliveryFailed` existe para preencher.

### 20.4 Mecanismo de despacho: reaproveitado, não reinventado

`OnDelivered` usa o `DispatchTopicEvent`/`TPipeTopicWork` que já existiam para `OnPublish`/
`OnTopicMessage` (mesmo tipo `TPipeTopicEvent`, zero código novo em `Pipes.Base.pas` além de
chamar o que já tinha). `OnDeliveryFailed` precisou de um par novo e simétrico —
`TPipeDeliveryFailedEvent`, `TPipeDeliveryFailedWork`, `qeDeliveryFailed` em
`TPipeQueuedKind`, `DispatchDeliveryFailedEvent` — porque a assinatura tem um campo a mais
(`AError`); a mecânica (drena por `FInFlight`, `pdmMainThread` via fila com guarda, os demais
modos via `EventPool`) é idêntica à de qualquer outro evento da lib, só reaproveita `FMsg` do
`TPipeQueuedEvent` (já existia para `OnError`) em vez de um campo string dedicado.

### 20.5 Testes: sucesso é determinístico; falha usa `MaxMessageSize`, não uma corrida de socket

Os três casos de sucesso (`OnDelivered_FanOut_UmaVezPorAssinante`,
`OnDelivered_Retido_ChegaComARetainedTrue`, `OnDelivered_PublishBatch_UmPorItemPorConexao`,
em `Pipes.PubSubTests`, dual) são caminho feliz normal, sem timing especial. O caso de falha
(`OnDeliveryFailed_PayloadExcedeMaxMessageSize`) **não** tenta reproduzir uma conexão morrendo
no meio exato do envio — essa corrida entre "o reader detecta a queda e remove a conexão" e
"o teste consegue publicar antes disso" é exatamente o tipo de teste flaky que §7 (T4/T5) e
§13.10 já registraram como armadilha. Em vez disso, o teste abre o servidor com
`MaxMessageSize` pequeno o bastante para deixar a assinatura passar (o filtro sozinho cabe)
mas pequeno demais para o envelope tópico+corpo da publicação: `PipeValidateMaxPayload`
recusa o `Write` ANTES de tocar o socket/pipe, de forma 100% determinística, exercitando o
mesmo bloco `except` que uma conexão morta exercitaria — sem depender de nenhuma corrida real
de SO. Verificado nos dois compiladores: FPC/Win64 (`PipesIntegrationTestsFpc.exe`, suíte
`TPipePubSubTests`, 27/27, dentro dos 126/126 da suíte de integração inteira) e Delphi/DUnitX
(147/147 unit + 126/126 integração, 0 leak/falha/erro, confirmado pelo usuário 2026-08-22).

### 20.6 Efeito colateral: um bug pré-existente encontrado e corrigido em `PublishBatch`

Escrever `OnDelivered_PublishBatch_UmPorItemPorConexao` expôs que `PublishBatch` gravava o
bit `PIPE_FLAG_RETAIN` do frame AO VIVO direto do campo `Retain` de cada
`TPipePublishItem`, em vez de sempre `False` como `FanOut`/`Publish` fazem (§9, "Sai SEMPRE
com PIPE_FLAG_RETAIN desligado"). Efeito: um item com `Retain = True` entregue a um
assinante JÁ conectado chegava com `ARetained = True` em `OnTopicMessage` — o cliente era
enganado sobre "isto é notícia ou histórico?", já que nenhum teste existente cobria um
assinante conectado NA HORA da publicação (o único teste de retain em lote assinava DEPOIS,
recebendo pelo replay de `SendRetained`, onde `ARetained = True` é correto). Corrigido junto
com esta feature: a linha que monta o `TPipeFrame` do lote agora passa `False` sempre,
igual `FanOut`; teste de regressão
`PublishBatch_RetainAoVivo_NaoMarcaARetainedParaQuemJaAssinava` cobre o caminho que faltava.

## 21. Connect assíncrono (`TPipeClient.ConnectAsync`)

`Connect(ATimeoutMs)` é síncrono: se o servidor não estiver no ar dentro do prazo, levanta
`EPipeTimeout`/`EPipeError` e o app fica com o problema. `AutoReconnect` não cobre esse
buraco — ele só entra em ação depois que uma sessão JÁ existiu e caiu, porque quem o dispara
é `ReaderFinished`, e `ReaderFinished` só roda a partir da reader thread de uma sessão viva.
Ou seja: **"o app sobe antes do servidor" não tinha resposta na lib**. Quem precisava disso
escrevia um laço de retry manual em volta de `Connect` — reinventando, pior, o espaçamento,
o teto de tentativas e o failover que a lib já tem prontos ali do lado.

`ConnectAsync` fecha exatamente esse caso: volta na hora, e uma thread interna insiste até o
servidor aparecer. O sucesso chega em `OnConnected`; a desistência, em `OnError`;
`Connecting` diz se ainda está tentando; `Disconnect` cancela.

### 21.1 Reaproveitar a thread de reconexão, não criar uma segunda máquina de estados

A decisão central: `ConnectAsync` **não tem thread nem máquina de estados própria**. Liga um
flag e dispara a MESMA `TPipeReconnectThread`.

Isso não é economia de código, é reconhecer que já era a mesma operação. `TryReopenSession`
literalmente É "conectar até o servidor responder, com espaçamento entre tentativas, teto de
tentativas e volta pela lista de failover" — o que muda entre reconectar e conectar pela
primeira vez é só a ORIGEM da chamada, não o que precisa acontecer. Duplicar teria custado
uma segunda cópia do espaçamento (`WaitBetweenRetries`), do teto (`FReconnectAttempts`/
`FGaveUp`), do avanço de `FAddrIndex` e — o pior — uma segunda thread capaz de instalar uma
sessão, com toda a coordenação nova que isso exigiria contra a primeira.

Vale contrastar com o precedente **oposto** de `ADDR0` (§19), onde `FAddresses`/
`FAddressOrder` foram duplicados de `FIdentities`/`FIdentityOrder` de propósito em vez de
generalizados: lá eram dois DADOS independentes que por acaso tinham o mesmo formato, e
unificá-los criaria acoplamento onde não havia nenhum. Aqui é uma OPERAÇÃO só, com dois
pontos de entrada. Generalizar acerta num caso e erra no outro — a pergunta certa não é
"parece igual?", é "é a mesma coisa?".

### 21.2 O gate: `ReopenAllowed`, nos DOIS lugares (esquecer o segundo é um bug silencioso)

Todo o ponto de contato entre a feature nova e o motor existente é uma função de uma linha:
...
`TryReopenSession` consultava `FAutoReconnect` em **dois** lugares, e os dois tiveram de
virar `ReopenAllowed`:

1. **Na entrada**, antes de tentar. Óbvio: sem isso, um `ConnectAsync` com
   `AutoReconnect = False` desistiria antes da primeira tentativa.
2. **Na rechecagem com a conexão JÁ ABERTA**, logo depois do `PipeConnect` voltar. Esse é o
   que passa despercebido, e o efeito de esquecê-lo é bem pior do que "não funciona": o
   `PipeConnect` teria sucesso e o endpoint recém-conectado seria fechado e descartado ali
   mesmo, em silêncio, sempre que `AutoReconnect = False`. `ConnectAsync` ficaria tentando
   para sempre contra um servidor que está no ar e aceitando.

Esse segundo ponto existe por um motivo real (uma decisão de parar pode chegar DURANTE o
`PipeConnect`; instalar a sessão depois disso anunciaria `OnConnected` para quem já pediu
para parar) — então ele não some, só passa a perguntar a coisa certa.
...
completa o handshake e só ENTÃO valida a cadeia, então um cliente com certificado recusado
vê conexão estabelecida seguida de queda imediata (o mesmo caso já documentado no cabeçalho
de `Pipes.Client.pas` e em §7). Com `AutoReconnect = False` e um `ConnectAsync` em voo:

1. `TryReopenSession` instala a sessão, dispara `OnConnected` e devolve `True`, zerando
   `FReconnecting`.
2. A sessão cai nesse mesmo instante. Como `AutoReconnect = False`, `ReaderFinished` **não**
   cria thread nova — quem tem de retomar é esta mesma thread.
3. Em `Execute`, o CAS em `FReconnecting` sucede (foi zerado no passo 1) e o laço continua.
4. ...mas `TryReopenSession` verifica `ReopenAllowed` logo na entrada. Se o flag tivesse
   sido limpo no passo 1, o gate estaria fechado e a tentativa abortaria **em silêncio**:
   `Connecting` em `False`, sem sessão nenhuma viva e sem erro em lugar nenhum.

Daí a regra: só limpa quem **detém o ciclo** no momento de sair. Em `Execute` isso é
explícito:
...
Sai por sessão de pé (`LConn`) ou por `Disconnect` deliberado (`LDelib`) → esta thread
encerrou o trabalho, limpa. Sai porque o **CAS falhou** → a sessão caiu neste instante e
`ReaderFinished` já criou uma `TPipeReconnectThread` NOVA (possível quando `AutoReconnect`
está ligado ao mesmo tempo que um `ConnectAsync` está em voo); quem limpa é aquela thread,
e limpar aqui apagaria `Connecting` com a tentativa ainda viva.

Duas notas sobre a forma:

- **O snapshot é único de propósito.** `LConn`/`LDelib` são lidos UMA vez e servem às duas
  decisões (sair do laço e limpar o flag). Reler `FConnected` no `if` de dentro abriria uma
  janela em que a sessão cai entre as duas leituras: o laço sai por "estava de pé" mas a
  limpeza não acontece, e com `AutoReconnect = False` não existe thread nova para limpar
  depois — `Connecting` ficaria `True` para sempre. A expressão é ponto a ponto equivalente
  à original (`not (A and B and C)` = `(not A) or (not B) or (not C)`), inclusive no
  curto-circuito que garante que o CAS só roda quando não há sessão de pé.
- **A saída de baixo do laço** (desistiu: deliberado ou esgotado) limpa `FConnectingAsync`
  ANTES de `FReconnecting`, porque é o segundo que solta o `WaitReconnectDone` do
  `Disconnect` — quem acorda de lá tem de já enxergar `Connecting = False`.

`Disconnect` também limpa o flag explicitamente, logo depois do `WaitReconnectDone`. Não é
redundância defensiva: sem isso, o cancelamento dependeria da ordem em que a thread zera os
dois flags, e `Connecting` poderia ler `True` por um instante DEPOIS de `Disconnect` ter
retornado — exatamente o que um app faria a pergunta para decidir se ainda está tentando.

### 21.4 Sem parâmetro de timeout: o orçamento é o da reconexão

`ConnectAsync` não tem `ATimeoutMs`. O orçamento é `MaxReconnectAttempts` ×
`ReconnectDelayMs`, as duas properties que já existiam — mesma recusa de §12.5 em criar
orçamento próprio "por um ganho marginal". Com `MaxReconnectAttempts = 0` (o padrão) tenta
para sempre, que é o comportamento que o caso de uso pede: o PDV liga antes da retaguarda e
espera.

Isso é um teto **aproximado** de tempo de parede, não exato — cada tentativa gasta até
`ReconnectDelayMs` no `PipeConnect` e o espaçamento consome o que sobrar do mesmo intervalo.
Quem precisa de prazo exato compõe: chama `Disconnect` de um timer. Inventar um orçamento
próprio exigiria conciliá-lo com os dois já existentes em cada tentativa, e o resultado
seria três números disputando o mesmo relógio.

O esgotamento chega em `OnError`, não como exceção — não há para quem levantar, o chamador
já voltou. A mensagem é condicional ao contexto (`'conexao inicial esgotada apos N
tentativas'` em vez de `'reconexao esgotada...'`): quem nunca teve sessão nenhuma lendo
"reconexão esgotada" no log procuraria uma queda que não houve.

### 21.5 `GetLifecycleLocked`: `Connecting` também tranca a configuração

`EnsureInactive` passou a consultar `GetLifecycleLocked` (virtual em `TPipeBase`, default
`GetActive`) em vez de `GetActive` direto. `TPipeClient` sobrescreve para
`FConnected or Connecting`.

O motivo é uma janela que **só existe** com ativação assíncrona: com `Connect()` síncrono o
chamador fica bloqueado dentro do próprio objeto e não tem como trocar `Address` no meio da
tentativa. Com `ConnectAsync` ele volta na hora, e nada impediria o app de mexer em
`Address`/`Transport`/`TlsOptions` enquanto a thread ainda tenta — a tentativa seguinte
miraria outro servidor, ou o mesmo com outro transporte, sem ninguém ter pedido.

Método novo em vez de mudar `GetActive`: `Connected`/`Active` são API pública com significado
estabelecido ("há sessão"), e alargá-lo para "há sessão ou estou tentando" faria
`Connected = True` sem sessão nenhuma — quebrando todo código que decide enviar por essa
property. `TPipeServer` herda o default e não muda em nada.

### 21.6 Nenhuma mudança em `Pipes.Transport.*`

Nenhum dos cinco backends de `PipeConnect` (Windows, POSIX, Tcp, Tls, Android) tem
mecanismo de cancelamento de um connect em progresso vindo de outra thread, e esta feature
**não inventou um**. Cancelar um `ConnectAsync` herda o mesmo trade-off que `Disconnect` já
aceita hoje para a reconexão automática, e que `WaitReconnectDone` documenta desde sempre:
"pior caso, espera um `PipeConnect` de até `ReconnectDelayMs` terminar".

Isso é uma escolha, não um esquecimento. Cancelamento real exigiria mexer nos cinco backends
de plataforma — o ponto de mais alto risco do projeto, sem precedente e com verificação
Android e POSIX que esta máquina não consegue rodar (§13, §16) — para transformar uma espera
de milissegundos-a-`ReconnectDelayMs` numa de zero. O teste de cancelamento afirma o que a
lib de fato garante (**para de tentar em no máximo uma tentativa em curso**), e não um "volta
instantâneo" que seria mentira.

### 21.7 Testes

Nove testes de integração em `Pipes.ConnectAsyncTests` (DUnitX + FPCUnit espelhados),
sempre com espera por deadline (`WaitCount`/`WaitClientCount`/`WaitNotConnecting`), nunca
`Sleep` fixo como asserção. `WaitNotConnecting` existe porque `Connecting` cai um passo
DEPOIS de `OnConnected` — quem limpa o flag é a thread de reconexão, já fora de
`TryReopenSession`.

| Teste | O que guarda |
|-------|--------------|
| `ConnectAsync_ServidorJaNoAr_ConectaENotifica` | caminho trivial: não bloqueia nem quando o servidor já está lá |
| `ConnectAsync_ServidorSobeDepois_ConectaQuandoAparece` | **o teste central**: `Connecting=True`/`Connected=False` logo após a chamada, servidor sobe depois, `OnConnected` dispara |
| `ConnectAsync_ServidorNuncaAparece_EsgotaComMensagemDeConexaoInicial` | esgota via `OnError` com a mensagem de conexão INICIAL (não "reconexão"), `Connecting` cai, `OnConnected` nunca dispara |
| `ConnectAsync_CanceladoPorDisconnect_ParaDeTentar` | `Disconnect` para o laço infinito dentro de uma tentativa em curso, e continua parado 1,5s depois (§21.6) |
| `ConnectAsync_ChamadoDeNovoEnquantoEmVoo_ReiniciaSemDuplicar` | rechamada cancela a anterior; uma thread órfã apareceria como um SEGUNDO `OnConnected`/`ClientCount = 2` |
| `ConnectAsync_ComFailoverAddresses_MigraParaBackupSemPrimario` | avanço por endereço na lista, a MESMA regra da reconexão |
| `ConnectAsync_ComAutoReconnect_ContinuaReconectandoDepois` | é aqui que um `FConnectingAsync` vazado (ou limpo cedo demais) apareceria: depois do `ConnectAsync` cumprir o papel, o ciclo de vida normal do `AutoReconnect` segue intacto |
| `Connecting_SemChamarConnectAsync_PermaneceFalse` | sanidade: `Connect()` síncrono nunca liga `Connecting` |
| `ConnectAsync_EmVoo_ImpedeTrocarTransportOuAddress` | `GetLifecycleLocked`/`EnsureInactive` (§21.5), e a trava sai junto com o `Disconnect` |

Os dois de risco (cancelamento e convivência com `AutoReconnect`) foram rodados cinco vezes
seguidas em isolamento, com tempo de suíte estável (~5,7s), pela mesma razão registrada em
§13.10: um teste de concorrência que passa uma vez não provou nada.

### 21.8 Milestones

| # | Milestone | Conteúdo | Status |
|---|-----------|----------|--------|
| CONN0a | `Pipes.Base` | `GetLifecycleLocked` virtual; `EnsureInactive` passa a consultá-lo | concluído |
| CONN0b | `Pipes.Client` (motor) | `FConnectingAsync`, `ReopenAllowed` nos DOIS gates de `TryReopenSession`, limpeza condicional em `Execute` e em `Disconnect`, mensagem de esgotamento condicional | concluído |
| CONN0c | `Pipes.Client` (API) | `ConnectAsync`, `Connecting`, `GetLifecycleLocked` override | concluído |
| CONN0d | Testes | 9 testes de integração nos dois frameworks | concluído |

## 22. Diagnóstico de tentativa de conexão (`OnConnectAttemptFailed`)

Motivado por uso real: a aplicação opera em silêncio de propósito — o cliente continua
trabalhando durante uma queda, e se a reconexão der certo ninguém é interrompido. O problema
aparece depois, quando alguém relata "às 14h40 não estava funcionando direito" e o log não
tem o que dizer sobre aquela janela.

Com `AutoReconnect` (e agora `ConnectAsync`), a trilha de eventos até aqui era:
...
O miolo é o que interessa: foram 300 tentativas contra um servidor fora do ar, ou 3 tentativas
recusadas por certificado, ou o failover migrou para o backup e voltou? `OnConnectAttemptFailed`
preenche essa janela.

### 22.1 Evento novo, não `OnError`

Mandar tentativa-falhou para `OnError` teria sido de uma linha, e estaria errado por dois
motivos. Primeiro, mudaria o comportamento de **todo mundo** que já usa `AutoReconnect`: um app
que hoje mostra um balão vermelho no `OnError` passaria a mostrá-lo a cada `ReconnectDelayMs`.
Segundo, e mais de fundo, confunde dois significados: `OnError` é "algo que o app precisa
tratar", enquanto uma tentativa falha é o estado NORMAL de um cliente esperando o servidor —
a próxima tentativa já está a caminho e não há nada a decidir.

O desfecho definitivo continua nos eventos de sempre: `OnConnected` quando alguma tentativa
vinga, e o `OnError` de esgotamento (`'conexao inicial esgotada...'` / `'reconexao
esgotada...'`, §21.4) quando `MaxReconnectAttempts` é alcançado.

Sem handler atribuído o evento não custa nada — `DispatchAttemptFailedEvent` sai no primeiro
`if not Assigned(AEvent)`. Não há property booleana de opt-in: seria configuração sem função,
já que não atribuir o handler É o opt-out.

### 22.2 Dispara nos DOIS caminhos, de propósito

`ConnectAsync` e reconexão automática compartilham a mesma `TPipeReconnectThread` (§21.1), e o
evento sai do único ponto por onde toda tentativa passa — o `except` do `PipeConnect` em
`TryReopenSession`. Consequência: o mesmo evento cobre "nunca conectei" e "caí e estou
voltando".

Isso é o certo para a finalidade. Separar em dois eventos daria ao log um ponto cego
justamente no caso mais comum — a sessão que caiu no meio do expediente —, e obrigaria quem
loga a assinar os dois para ter a história completa. Quem quiser distinguir os casos tem
`Connecting` no próprio objeto.

Note que o evento também alcança quem **não** usa `ConnectAsync`: um cliente com `Connect`
síncrono + `AutoReconnect` recebe as tentativas de reconexão normalmente (é o que o teste
`AttemptFailed_ReconexaoAutomatica_TambemDispara` fixa).

### 22.3 `AAddress` por VALOR — a armadilha que o evento evita
...
O endereço vai por parâmetro, capturado em `LTentado` ANTES do `PipeConnect`, e não é lido de
`ActiveAddress` dentro do handler. A razão é sutil e cara: com `FailoverAddresses`, o
`FAddrIndex` avança **dentro do próprio `except`**, antes do evento sair. Um handler que
consultasse `Client.ActiveAddress` registraria o endereço da PRÓXIMA tentativa, não o que
acabou de falhar — um log forense que troca o culpado é pior que log nenhum, porque parece
correto.

O `except` também passou de `on EPipeError do` para `on E: EPipeError do`: a mensagem do
transporte é o que separa "servidor ausente" de "servidor recusou meu certificado", e era
justamente ela que estava sendo descartada.

`AAttempt` é o mesmo `FReconnectAttempts` que `MaxReconnectAttempts` limita — começa em 1 e
conta desde a última sessão DURÁVEL, não desde sempre (mesmo critério de §12.4). É o campo que
permite a quem loga aplicar uma política de decimação ("registrei a 1ª, a 5ª, a 20ª") sem
guardar estado próprio entre chamadas.

Não há `AConnId`, ao contrário de todos os outros eventos da lib: não existe conexão — é
justamente disso que o evento fala. Um parâmetro sempre-zero seria ruído.

### 22.4 Frequência: a lib não decima

Uma tentativa contra servidor ausente consome `ReconnectDelayMs` inteiro (`WinPipeConnect`
faz polling até o deadline, §21.6), então o evento sai a cada ~`ReconnectDelayMs` enquanto
durar a queda. A 3 s por tentativa, são ~20 eventos por minuto; uma loja com o servidor fora
a noite toda gera ~10 mil.

A lib **não** agrega, não decima e não tem janela de supressão. Isso é deliberado: a política
certa depende do domínio (arquivo com rotação? syslog? telemetria com custo por evento?), e
`AAttempt` já dá ao app tudo de que ele precisa para implementar a sua — tipicamente registrar
com intervalo crescente, acumulando o contador entre registros. Uma decimação embutida na lib
seria uma política a mais para configurar e um caso a mais para depurar quando o log parecesse
"faltar linhas".

### 22.5 Infraestrutura de despacho: espelhada, não inventada

`TPipeAttemptFailedWork` + `qeAttemptFailed` + `DispatchAttemptFailedEvent` em `Pipes.Base.pas`
espelham exatamente `TPipeDeliveryFailedWork`/`qeDeliveryFailed`/`DispatchDeliveryFailedEvent`
de §20 — mesmo contrato de `IncInFlight`/`DecInFlight`, mesmo caminho alternativo de
`pdmMainThread` via `TPipeQueuedEvent`/`TThread.Queue` com a guarda refcounted.

`TPipeQueuedEvent` ganhou campos próprios (`FAddress`, `FAttempt`) em vez de reaproveitar
`FTopic` como slot genérico de string, que é o que `qeSubscription` faz para o filtro. Dois
campos a mais num objeto criado só em `pdmMainThread` são baratos; um endereço de conexão
viajando num campo chamado `FTopic` cobraria caro do próximo leitor.

O evento sai da **thread de reconexão**, não da de leitura — o mesmo lugar de onde o
`DispatchError` de esgotamento já saía. Nenhuma invariante de threading nova (invariante 1
continua valendo: a thread de leitura não executa código de usuário).

### 22.6 Testes

Quatro testes em `Pipes.ConnectAsyncTests` (mesma fixture, dual DUnitX/FPCUnit — ela já monta
servidor primário + backup + cliente + contadores, e o evento dispara pelos dois caminhos que
a unit já exercita):

| Teste | O que guarda |
|-------|--------------|
| `AttemptFailed_ServidorAusente_DisparaPorTentativaComContador` | um evento por tentativa (exatamente 3 com teto 3), `AAttempt` = `1\|2\|3`, endereço correto em todos, mensagem de erro não vazia, e PARA de disparar ao esgotar |
| `AttemptFailed_ComFailover_ReportaOEnderecoQueFalhou` | §22.3: o evento reporta o PRIMÁRIO, embora `ActiveAddress` já aponte para o backup |
| `AttemptFailed_ReconexaoAutomatica_TambemDispara` | §22.2: alcança quem usa `Connect` síncrono + `AutoReconnect`, sem `ConnectAsync` nenhum |
| `AttemptFailed_ConexaoBemSucedida_NaoDispara` | operação silenciosa: conexão que dá certo de primeira não gera ruído |

### 22.7 Milestones

| # | Milestone | Conteúdo | Status |
|---|-----------|----------|--------|
| DIAG0a | `Pipes.Types` | `TPipeAttemptFailedEvent` | concluído |
