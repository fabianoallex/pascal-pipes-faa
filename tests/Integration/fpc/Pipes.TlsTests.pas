...
type
  ...
  published
    // --- caminho feliz ---
    procedure Tls_RoundTripCifrado;
    procedure Mtls_ClienteComCertDaCa_Conecta;
    // --- reconexao ---
    procedure Mtls_AutoReconnect_RefazHandshakeAposQueda;
    // --- validacao do servidor pelo cliente (default seguro) ---
    procedure Tls_ClienteValidaServidorPorPadrao_Recusa;
    procedure Tls_ValidaServidorPorIp_Aceita;
    procedure Tls_ValidaServidorPorNome_Aceita;
    // --- recusa (o que de fato prova que ha autenticacao) ---
    procedure Mtls_ClienteSemCert_Recusado;
    procedure Mtls_ClienteDeOutraCa_Recusado;
    procedure Mtls_ClienteAutoAssinado_Recusado;
    procedure Mtls_ClienteDeCaGemea_Recusado;
    procedure Handshake_ClienteMudo_EstouraNoPrazo;
...
begin
  inherited;
  if PkiDir = '' then
    Fail('tests/pki nao encontrada a partir de ' + ParamStr(0) +
...
begin
  ...
  AssertException('Listen sem certificado deveria falhar', EPipeTls,
...
begin
  ...
  AssertException('trocar CertFile com o servidor ativo deveria levantar',
    EPipeError, DoTrocaCertComServidorAtivo);
end;

initialization
  RegisterTest(TPipeTlsTests);
